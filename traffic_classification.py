...
# gRPC通道
channel = implementations.insecure_channel(conf['grpc']['host'], conf.getint('grpc', 'port'))
...
def sendRes(fname, type):
    # TODO 修改地址和端口号
    s.sendto(fname.encode('utf-8'), (conf['socket_client']['host'], conf.getint('socket_client', 'port')))
    s.sendto(type.encode('utf-8'), (conf['socket_client']['host'], conf.getint('socket_client', 'port')))
...
def main():
    try:
        while True:
            ...
            print('服务器正在 '+ ip + ':'+ conf['socket_server']['port'] +' 运行，等待客户端连接...')
            sock, addr = s.accept()
